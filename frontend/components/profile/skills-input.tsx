...
const selectClass =
  'rounded-[6px] border border-[var(--border)] bg-[var(--surface)] px-2 py-1 text-xs text-[var(--muted2)] focus:border-[var(--border-bright)] focus:outline-none disabled:cursor-not-allowed';
  ...
  return (
    <div className="space-y-2">
      {value.map((skill, i) => (
        <div
          key={`${skill.name}-${i}`}
          className="flex items-center gap-2.5 rounded-[8px] border border-[var(--border)] bg-[var(--card)] px-3 py-2.5"
        >
        ...
        <>
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-[8px] border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--text)] placeholder:text-[var(--muted)] focus:border-[var(--border-bright)] focus:outline-none disabled:cursor-not-allowed"
            />
            <select
              value={draftYears}
              onChange={(e) => setDraftYears(Number(e.target.value))}
              disabled={disabled}
              className="rounded-[8px] border border-[var(--border)] bg-[var(--card)] px-2 py-2 text-sm text-[var(--muted2)] focus:border-[var(--border-bright)] focus:outline-none disabled:cursor-not-allowed"
            >
              {YEAR_OPTIONS.map((y) => (
                <option key={y} value={y}>
                  {y} yr{y > 1 ? 's' : ''}
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-[8px] border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--muted2)] transition-colors hover:border-[var(--border-bright)] hover:text-[var(--text)] disabled:cursor-not-allowed disabled:opacity-50"
            >
